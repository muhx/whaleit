...
key_files:
  ...
  modified:
    ...
    - README.md
decisions:
  - Auth salt strings in auth.rs left unchanged to avoid invalidating existing
    tokens/secrets (per threat model T-02-03)
  - Internal crate names (whaleit-*) left unchanged per D-06/D-07
  - Twitter handle @WhaleitApp kept as-is (external service URL, not under our
    control)
  - Docker binary renamed from whaleit-server to whaleit-server (user-facing in
    Docker)
  - Database filename whaleit.db kept as-is in Docker/compose to avoid breaking
...
All user-facing "Whaleit" strings in backend (Rust), configuration, and
documentation renamed to "WhaleIt". Internal crate references remain whaleit-\*
unchanged.
...
- **apps/tauri/tauri.conf.json**: productName, mainBinaryName → "WhaleIt";
  identifier → "com.whaleit.app"; deep-link schemes → "whaleit"; updater
  endpoint → whaleit.app; window title → "WhaleIt"
- **apps/tauri/gen/apple/project.yml**: bundleIdPrefix → "com.whaleit";
  PRODUCT_NAME → "WhaleIt"; PRODUCT_BUNDLE_IDENTIFIER → "com.whaleit.app"
- **apps/tauri/src/menu.rs**: App menu label, about dialog, update message,
  support email → WhaleIt
...
- **apps/server/src/api/settings.rs**: Backup filename prefix →
  "whaleit*backup*"
...
- **README.md**: Full rebrand — title, tagline ("Your Friendly Finance
  Companion"), GitHub URLs (muhx/whaleit), website (whaleit.app), Docker
...
- **Found during:** task 1
- **Issue:** Plan missed support@whaleit.app email in report_issue handler
  (line 63)
...
All 13 modified files exist. Both task commits (7ae51160, 33437d1e) verified in
git log.
