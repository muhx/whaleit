...
/**
 * Store WhaleIt Connect tokens in the backend's encrypted secret store.
...
/**
 * Clear WhaleIt Connect session from the backend's secret store.
 * Works in both desktop (Tauri) and web modes.
 */
/**
 * Restore WhaleIt Connect session from the backend (web mode only).
