...
interface WhaleItConnectContextValue {
  isEnabled: boolean;
  isConnected: boolean;
...
const disabledContextValue: WhaleItConnectContextValue = {
  isEnabled: false,
  isConnected: false,
...
function EnabledWhaleItConnectProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<WhaleItConnectContextValue>(
    () => ({
      isEnabled: true,
      isConnected: !!session,
      isInitializing,
...
export function WhaleItConnectProvider({ children }: { children: ReactNode }) {
  ...
  if (!CONNECT_ENABLED || !isCloudSyncAvailable) {
    return (
      <WhaleItConnectContext.Provider value={disabledContextValue}>
        {children}
      </WhaleItConnectContext.Provider>
    );
  }

  return <EnabledWhaleItConnectProvider>{children}</EnabledWhaleItConnectProvider>;
}

export const useWhaleItConnect = () => {
  const ctx = useContext(WhaleItConnectContext);
  if (!ctx) {
    throw new Error("useWhaleItConnect must be used within a WhaleItConnectProvider");
  }
  return ctx;
};
