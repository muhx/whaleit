//! PostgreSQL settings repository implementation.

use async_trait::async_trait;
use diesel::prelude::*;
use diesel_async::RunQueryDsl;
use std::sync::Arc;

use super::model::AppSettingDB;
use crate::db::PgPool;
use crate::errors::StoragePgError;
use crate::schema::app_settings::dsl::*;
use crate::schema::{accounts, assets};
use whaleit_core::assets::AssetKind;
use whaleit_core::errors::Result;
use whaleit_core::settings::{Settings, SettingsRepositoryTrait, SettingsUpdate};

pub struct PgSettingsRepository {
    pool: Arc<PgPool>,
}

impl PgSettingsRepository {
    pub fn new(pool: Arc<PgPool>) -> Self {
        PgSettingsRepository { pool }
    }
}

#[async_trait]
impl SettingsRepositoryTrait for PgSettingsRepository {
    async fn get_settings(&self) -> Result<Settings> {
        let mut conn = self.pool.get().await.map_err(|e| StoragePgError::from(e))?;
        let all_settings: Vec<(String, String)> = app_settings
            .select((setting_key, setting_value))
            .load::<(String, String)>(&mut conn)
            .await
            .map_err(StoragePgError::from)?;

        let mut settings = Settings::default();

        for (key, value) in all_settings {
            match key.as_str() {
                "theme" => settings.theme = value,
                "font" => settings.font = value,
                "base_currency" => settings.base_currency = value,
                "timezone" => settings.timezone = value,
                "instance_id" => settings.instance_id = value,
                "onboarding_completed" => {
                    settings.onboarding_completed = value.parse().unwrap_or(false);
                }
                "auto_update_check_enabled" => {
                    settings.auto_update_check_enabled = value.parse().unwrap_or(true);
                }
                "menu_bar_visible" => {
                    settings.menu_bar_visible = value.parse().unwrap_or(true);
                }
                "sync_enabled" => {
                    settings.sync_enabled = value.parse().unwrap_or(true);
                }
                _ => {}
            }
        }

        Ok(settings)
    }

    async fn update_settings(&self, new_settings: &SettingsUpdate) -> Result<()> {
        let mut conn = self.pool.get().await.map_err(|e| StoragePgError::from(e))?;
        let settings = new_settings.clone();

        if let Some(ref theme) = settings.theme {
            diesel::insert_into(app_settings)
                .values(&AppSettingDB {
                    setting_key: "theme".to_string(),
                    setting_value: theme.clone(),
                })
                .on_conflict(setting_key)
                .do_update()
                .set(setting_value.eq(theme.clone()))
                .execute(&mut conn)
                .await
                .map_err(StoragePgError::from)?;
        }

        if let Some(ref font) = settings.font {
            diesel::insert_into(app_settings)
                .values(&AppSettingDB {
                    setting_key: "font".to_string(),
                    setting_value: font.clone(),
                })
                .on_conflict(setting_key)
                .do_update()
                .set(setting_value.eq(font.clone()))
                .execute(&mut conn)
                .await
                .map_err(StoragePgError::from)?;
        }

        if let Some(ref base_currency) = settings.base_currency {
            diesel::insert_into(app_settings)
                .values(&AppSettingDB {
                    setting_key: "base_currency".to_string(),
                    setting_value: base_currency.clone(),
                })
                .on_conflict(setting_key)
                .do_update()
                .set(setting_value.eq(base_currency.clone()))
                .execute(&mut conn)
                .await
                .map_err(StoragePgError::from)?;
        }

        if let Some(ref timezone) = settings.timezone {
            diesel::insert_into(app_settings)
                .values(&AppSettingDB {
                    setting_key: "timezone".to_string(),
                    setting_value: timezone.clone(),
                })
                .on_conflict(setting_key)
                .do_update()
                .set(setting_value.eq(timezone.clone()))
                .execute(&mut conn)
                .await
                .map_err(StoragePgError::from)?;
        }

        if let Some(onboarding_completed) = settings.onboarding_completed {
            diesel::insert_into(app_settings)
                .values(&AppSettingDB {
                    setting_key: "onboarding_completed".to_string(),
                    setting_value: onboarding_completed.to_string(),
                })
                .on_conflict(setting_key)
                .do_update()
                .set(setting_value.eq(onboarding_completed.to_string()))
                .execute(&mut conn)
                .await
                .map_err(StoragePgError::from)?;
        }

        if let Some(auto_update_check_enabled) = settings.auto_update_check_enabled {
            diesel::insert_into(app_settings)
                .values(&AppSettingDB {
                    setting_key: "auto_update_check_enabled".to_string(),
                    setting_value: auto_update_check_enabled.to_string(),
                })
                .on_conflict(setting_key)
                .do_update()
                .set(setting_value.eq(auto_update_check_enabled.to_string()))
                .execute(&mut conn)
                .await
                .map_err(StoragePgError::from)?;
        }

        if let Some(menu_bar_visible) = settings.menu_bar_visible {
            diesel::insert_into(app_settings)
                .values(&AppSettingDB {
                    setting_key: "menu_bar_visible".to_string(),
                    setting_value: menu_bar_visible.to_string(),
                })
                .on_conflict(setting_key)
                .do_update()
                .set(setting_value.eq(menu_bar_visible.to_string()))
                .execute(&mut conn)
                .await
                .map_err(StoragePgError::from)?;
        }

        if let Some(sync_enabled) = settings.sync_enabled {
            diesel::insert_into(app_settings)
                .values(&AppSettingDB {
                    setting_key: "sync_enabled".to_string(),
                    setting_value: sync_enabled.to_string(),
                })
                .on_conflict(setting_key)
                .do_update()
                .set(setting_value.eq(sync_enabled.to_string()))
                .execute(&mut conn)
                .await
                .map_err(StoragePgError::from)?;
        }

        Ok(())
    }

    async fn get_setting(&self, setting_key_param: &str) -> Result<String> {
        let mut conn = self.pool.get().await.map_err(|e| StoragePgError::from(e))?;
        let result = app_settings
            .filter(setting_key.eq(setting_key_param))
            .select(setting_value)
            .first(&mut conn)
            .await;

        match result {
            Ok(value) => Ok(value),
            Err(diesel::result::Error::NotFound) => {
                let default_value = match setting_key_param {
                    "theme" => "light",
                    "font" => "font-mono",
                    "timezone" => "",
                    "onboarding_completed" => "false",
                    "auto_update_check_enabled" => "true",
                    "menu_bar_visible" => "true",
                    "sync_enabled" => "true",
                    _ => return Err(StoragePgError::from(diesel::result::Error::NotFound).into()),
                };
                Ok(default_value.to_string())
            }
            Err(e) => Err(StoragePgError::from(e).into()),
        }
    }

    async fn update_setting(
        &self,
        setting_key_param: &str,
        setting_value_param: &str,
    ) -> Result<()> {
        let mut conn = self.pool.get().await.map_err(|e| StoragePgError::from(e))?;

        diesel::insert_into(app_settings)
            .values(AppSettingDB {
                setting_key: setting_key_param.to_string(),
                setting_value: setting_value_param.to_string(),
            })
            .on_conflict(setting_key)
            .do_update()
            .set(setting_value.eq(setting_value_param))
            .execute(&mut conn)
            .await
            .map_err(StoragePgError::from)?;

        Ok(())
    }

    async fn get_distinct_currencies_excluding_base(
        &self,
        base_currency_param: &str,
    ) -> Result<Vec<String>> {
        let mut conn = self.pool.get().await.map_err(|e| StoragePgError::from(e))?;

        let currency_assets: Vec<String> = assets::table
            .filter(assets::kind.eq(AssetKind::Fx.as_db_str()))
            .filter(assets::quote_ccy.ne(base_currency_param))
            .select(assets::quote_ccy)
            .distinct()
            .load::<String>(&mut conn)
            .await
            .map_err(StoragePgError::from)?;

        let account_currencies: Vec<String> = accounts::table
            .filter(accounts::currency.ne(base_currency_param))
            .select(accounts::currency)
            .distinct()
            .load::<String>(&mut conn)
            .await
            .map_err(StoragePgError::from)?;

        let mut all_currencies: Vec<String> = Vec::new();
        all_currencies.extend(currency_assets);
        all_currencies.extend(account_currencies);
        all_currencies.sort();
        all_currencies.dedup();

        Ok(all_currencies)
    }
}
